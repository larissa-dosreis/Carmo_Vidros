...
function initLogin() {
    ...
    form.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            btn.disabled = false;
            btn.innerHTML = '<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2"><path d="M15 3h4a2 2 0 012 2v14a2 2 0 01-2 2h-4"/><polyline points="10 17 15 12 10 7"/><line x1="15" y1="12" x2="3" y2="12"/></svg> Entrar';
        }
    });
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (document.querySelector('.admin-dashboard')) {
        ...
        initMaterialForm();
        loadProdutos();
        loadLeads(); // <--- ADICIONE ESTA LINHA AQUI
    }
});
...
function renderLeads(leads) {
    ...
    if (leads.length === 0) {
        tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding: 20px; color: #666;">Nenhum lead recebido ainda.</td></tr>';
        return;
    }

    tbody.innerHTML = leads.map(lead => {
        // Remove tudo que não for número do telefone (para garantir que o link do zap funcione)
        const apenasNumeros = String(lead.telefone).replace(/\D/g, '');
        
        // Garante que tenha o código do Brasil na frente, caso o cliente não tenha colocado
        const zapNumber = apenasNumeros.length <= 11 ? `55${apenasNumeros}` : apenasNumeros;
        
        // Cria a mensagem automática
        const textoMsg = `Olá ${lead.nome}, recebi seu pedido de orçamento para *${lead.interesse}* pelo nosso site. Como posso te ajudar hoje?`;
        const linkWhatsApp = `https://wa.me/${zapNumber}?text=${encodeURIComponent(textoMsg)}`;

        return `
            <tr>
                <td style="color:#666; font-size: 13px;">${lead.data}</td>
                <td style="font-weight: 500;">${lead.nome}</td>
                <td>${lead.telefone}</td>
                <td>
                    <span style="background: #e3f2fd; color: #0d47a1; padding: 4px 8px; border-radius: 4px; font-size: 12px; font-weight: 500;">
                        ${lead.interesse}
                    </span>
                </td>
                <td>
                    <a href="${linkWhatsApp}" target="_blank" style="display: inline-flex; align-items: center; gap: 6px; background: #25D366; color: white; padding: 6px 12px; border-radius: 6px; text-decoration: none; font-size: 13px; font-weight: 600; transition: 0.2s;">
